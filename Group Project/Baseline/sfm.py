...
class SFM(object): 
    def __init__(self, opts): 
        ...
        if opts.calibration_mat == 'benchmark': 
            ...
            self.K = np.array([[2862.07,0,1802.81],[0,2860.28,1363.97],[0,0,1]])
        ...
        def _GetColors(): 
            ...
            for k in self.image_data.keys(): 
                ...
                for ext in ['.jpeg', '.jpg', '.png']:
                    possible_path = os.path.join(self.images_dir, k + ext)
                    if os.path.exists(possible_path):
                        image_path = possible_path
                        break

                if image_path is None:
                    print(f"No image found for {k} with supported extensions.")
                    continue  # Skip this image if no file is found
    ...
    def _ComputeReprojectionError(self, name): 
        ...
        if self.opts.plot_error: 
            ...
            image_path = None
            for ext in ['.jpeg', '.jpg', '.png']:
                possible_path = os.path.join(self.images_dir, name + ext)
                if os.path.exists(possible_path):
                    image_path = possible_path
                    break

            if image_path is None:
                print(f"No image found for {name} with supported extensions.")
...
def SetArguments(parser): 
    ...
    parser.add_argument('--ext',action='store',type=str,default='jpg,png,jpeg,JPG',dest='ext', 
